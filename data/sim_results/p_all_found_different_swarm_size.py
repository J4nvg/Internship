...
try:
    ...
    print(f"FATAL ERROR: Could not find the file '{SUMMARY_FILENAME}'.")
    print("Please save your data to that file or update the SUMMARY_FILENAME variable.")
    sys.exit(1)
...
for i, (tactic_name, data) in enumerate(results.items()):
    if not len(data['swarm_sizes']):
        print(f"Skipping plot for {tactic_name} (no data points).")
        continue
    ...
    ax.plot(
        ...
        color=color,
        linewidth=2,
        marker='o', # Add markers to show the discrete data points
        markersize=8
    )
...
# Set x-ticks to only the swarm sizes we have data for
...
save_filename = (
    f'prob_all_hiders_found_vs_SwarmSize_'
